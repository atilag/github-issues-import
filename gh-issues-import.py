...
http_error_messages[403] = "ERROR: Github has denied last request because of their abuse policy!"
...
def init_config():
	...
	arg_parser.add_argument('--migrate-users',    dest='migrate_users',    action='store_true', help="Migrate source users to target users.")
	...
	config.set('settings', 'migrate-users', str(not args.migrate_users))
	...
	def get_server_for(which):
		...
		config.set(which, 'api_url' api_url)
	...
	def get_credentials_for(which):
		if config.has_option(which, 'token'):
			return
...
def send_request(which, url, post_data=None, use_api_url=False):

	if post_data is not None:
		post_data = json.dumps(post_data).encode("utf-8")

	base_url = if use_api_url config.get(which, 'api_url') else	config.get(which, 'api')
	full_url = "%s/%s" % base_url, url)
	token = config.get(which, 'token', fallback=None)
	if token is not None:
		separator = '&' if '&' in full_url else '?'
		full_url = full_url + separator + "access_token=%s" % token
	...
	try:
		print("JUAN:" + req.full_url)
		print("JUAN:" + str(req.header_items()))
...
def import_comments(comments, issue_number):
	...
	for comment in comments:
		...
		if config.getboolean('settings', 'migrate-users'):
			change_source_users_to_target_users(template_data)
...
def change_source_users_to_target_users(template_data):
	source_user_email = ''
	if template_data['user_name'] in change_source_users_to_target_users.users:
		source_user_email = template_data['user_name']

	if source_user_email == '':
		source_user_query = 'users/%s' % template_data['user_name']
		source_user_info = send_request('source', user_query, use_api_url=True)
